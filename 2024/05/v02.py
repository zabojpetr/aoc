...
def order_update(data: typing.List[int], rules: typing.Dict[int,typing.Tuple[typing.List[int], typing.List[int]]]) -> typing.List[int]:
    right_order = []
    ...
    while len(data) > 0:
        ...
        rest = data[1:]
        if p not in rules:
            right_order.append(p)
            data.remove(p)
        else:
            while len(rest) > 0:
                before = rules[p][0]
                must_before = set(rest).intersection(before)
                if must_before:
                    p = list(must_before)[0]
                    rest.remove(p)
                else:
                    break
            right_order.append(p)
...
def main() -> None:
    ...
    wrong_updates = list(filter(lambda x: not is_correct(x, rules), updates))

    right_updates = list(map(lambda x: order_update(x, rules), wrong_updates))
