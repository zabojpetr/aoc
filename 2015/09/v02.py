...
def longest_cycle(place: str, edges: typing.Dict[str, typing.List[Edge]], used:set) -> int:
    maximum = 0
    first = True
    for dir in edges[place]:
        if dir.to in used:
        ...
        used.add(dir.to)
        sub_maximum = dir.len + longest_cycle(dir.to, edges, used)
        if first:
            maximum = sub_maximum
        else:
            maximum = max(maximum, dir.len + longest_cycle(dir.to, edges, used))
        used.remove(dir.to)
...
def main():
    ...
    with open("input.txt", "r") as fs:
        ...
        for c in edges:
            ...
            sub_max = longest_cycle(c,edges,{c})
            if first:
                minimum = sub_min
                maximum = sub_max
            else:
